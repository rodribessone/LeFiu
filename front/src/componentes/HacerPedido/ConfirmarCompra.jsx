...
export default function ConfirmarCompra() {
  ...
  const calculateTotal = () => {
    return cartItems.reduce((accumulator, item) => accumulator + item.precio * item.quantity, 0);
  };
  ...
  const handleSubmit = (event) => {
    ...
    const productos = cartItems
      .map((item) => `${item.nombre} (Cantidad: ${item.quantity}, Precio: $${item.precio})`)
      .join(", ");
  ...
  const isFormValid = nombre && direccion && medioPago && (medioPago !== "MercadoPago" || comprobante);
                ...
                {comprobante && (
                  <div className="mt-2">
                    {comprobante.type.startsWith("image/") ? (
                      ...
                      <p>Archivo PDF subido: {comprobante.name}</p>
                    )}
                  </div>
                )}
              </div>
